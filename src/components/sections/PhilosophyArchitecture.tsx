...
export default function PhilosophyArchitecture() {
  ...
  return (
    <div className="w-full h-full flex flex-col items-center justify-center p-8 max-w-5xl mx-auto">
      ...
      >
        <h2 className="text-4xl font-bold mb-4 text-white">No Somos un Data Broker</h2>
      ...
      <div className="w-full flex flex-col md:flex-row gap-6 h-[250px]">
        {/* Tabs Sidebar */}
        <div className="flex flex-col gap-3 w-full md:w-1/3">
          {PILLARS.map((pillar) => (
            <button
              ...
              className={`flex items-center gap-4 p-4 rounded-2xl transition-all duration-300 border text-left group ${
                activeTab === pillar.id 
            ...
            >
              ...
              <span className={`font-bold text-sm ${activeTab === pillar.id ? 'text-white' : 'text-text-muted group-hover:text-white'}`}>
                {pillar.title}
              </span>
              {activeTab === pillar.id && (
        ...
        {/* Tab Content */}
        <div className="flex-1 glassmorphism rounded-3xl border border-white/10 bg-slate-void/80 p-8 relative overflow-hidden flex items-center shadow-2xl">
          ...
          <AnimatePresence mode="wait">
            ...
            >
              <div className="inline-flex p-3 rounded-2xl bg-cyan-palantir/20 border border-cyan-palantir/30 mb-6">
                <activeData.icon className="w-8 h-8 text-cyan-palantir" />
              </div>
